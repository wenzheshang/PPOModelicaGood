...
fig,axs = plt.subplots(3,1,figsize=(20,24), gridspec_kw={'height_ratios': [1,1,1]})
plt.subplots_adjust(left=0.05, bottom=0.05, right=0.95, top=0.95, wspace=None, hspace=0.3)
...
axs[1].set_xticks(np.arange(0,166,5))
axs[1].set_xticklabels(np.arange(0,166,5),rotation = 30, fontsize = font_tick1)
axs[1].set_yticks(np.arange(0,0.31,0.05))
axs[1].set_yticklabels(['0%','5%','10%','15%','20%','25%','30%'],fontsize = font_tick1)
axs[1].set_xlim(-0.5,165.5)
axs[1].set_ylim(0,0.31)
axs[1].set_xlabel('source',fontsize = font_label)
...
led2 = axs[1].bar(x1,source_win_open,width = 5, align = 'edge',edgecolor = 'black',color= (72/255,120/255,208/255),
 label = 'source emission with window opening',zorder = 10)

led22 = axs[1].plot(x1+2.5,source_win_open,color = 'red',zorder = 10)

for a1,b1,i1 in zip(x1,source_win_open,range(len(x1))): # zip 函数
    axs[1].text(a1+0.3,b1+0.01,'%0.1f'%(source_win_open[i1]*100)+'%',fontsize=font_num,zorder = 15)

axs[1].legend(loc='upper right',prop={'size': font_legend},frameon=False)
axs[1].grid(linestyle = '--',zorder = 0)


axs[2].set_xticks(np.arange(0,101,5))
axs[2].set_xticklabels(np.arange(0,101,5),fontsize = font_tick1)
...
led3 = axs[2].bar(x2,source_win_close,width = 5, align = 'edge',edgecolor = 'black',color= (231/255,138/255,195/255),
...
for a2,b2,i2 in zip(x2,source_win_close,range(len(x2))): # zip 函数
    axs[2].text(a2+0.3,b2+0.01,'%0.1f'%(source_win_close[i2]*100)+'%',fontsize=font_num,zorder = 15)

axs[2].legend(loc='upper right',prop={'size': font_legend},frameon=False)
...
axs[0].set_xticks(np.arange(0,10.1,0.5))
axs[0].set_xticklabels(np.arange(0,10.1,0.5),fontsize = font_tick1)
axs[0].set_yticks(np.arange(0,0.71,0.1))
axs[0].set_yticklabels(['0%','10%','20%','30%','40%','50%','60%','70%'],fontsize = font_tick1)
axs[0].set_xlim(0,10.1)
axs[0].set_ylim(0,0.71)
...
led3 = axs[0].bar(x3,arf_win_open,width = 0.5, align = 'edge',edgecolor = 'black',color= (90/255,169/255,162/255),
 label = 'air exchange rate',zorder=10)


led33 = axs[0].plot(x3+0.25,arf_win_open,color = 'blue',zorder = 10)

for a3,b3,i3 in zip(x3,arf_win_open,range(len(x3))): # zip 函数
    axs[0].text(a3+0.03,b3+0.01,'%0.1f'%(arf_win_open[i3]*100)+'%',fontsize=font_num,zorder = 15)

axs[0].legend(loc='upper right',prop={'size': font_legend},frameon=False)
axs[0].grid(linestyle='--',zorder=0)

plt.savefig('F:\\Thinking\\AI\\arf\\paper\\arfvert.svg',dpi=900,format = 'svg')
