...
#数据读入
file_loc = 'D:\\IAQresult\\v01\\KPL8Z9_strategy2\\KPL8Z9\\00223--important\\result.xls'
...
for i in range(0,30):
    ...
    ax1.set_yticks(np.arange(0,max(pm_out[i*48:i*48+48])+25,max(pm_out[i*48:i*48+48])/5))
    ax1.set_yticklabels(np.round(np.arange(0,max(pm_out[i*48:i*48+48])+25,max(pm_out[i*48:i*48+48])/5),2),fontsize=font_tick)
    #设置横轴 特定x值时显示刻度
    ...
    for j in range(47):
        if window[i*48+j]!=window[i*48+j+1]:
            ax2.plot(x[j:j+2],[window[i*48+j],window[i*48+j]],'g',color = 'grey')
            ax2.plot([x[j+1],x[j+1]],window[i*48+j:i*48+j+2],'g',color = 'grey')
        if window[i*48+j]==window[i*48+j+1]:
            ax2.plot(x[j:j+2],window[i*48+j:i*48+j+2],'g',color = 'grey')
    ax2.plot(x[0],0,'g',color = 'grey',label="window status")
    ...
    for j in range(47):
        if action_base[i*48+j] != action_base[i*48+j+1]:
            ax3.plot(x[j:j+2],[action_base[i*48+j],action_base[i*48+j]],color = 'green')
            ax3.plot([x[j+1],x[j+1]],action_base[i*48+j:i*48+j+2],color = 'green')
        if action_base[i*48+j] == action_base[i*48+j+1]:
            ax3.plot(x[j:j+2],action_base[i*48+j:i*48+j+2],color = 'green')
    for j in range(47):
        if action_opt[i*48+j] != action_opt[i*48+j+1]:
            ax3.plot(x[j:j+2],[action_opt[i*48+j],action_opt[i*48+j]],color = 'red')
            ax3.plot([x[j+1],x[j+1]],action_opt[i*48+j:i*48+j+2],color = 'red')
        if action_opt[i*48+j] == action_opt[i*48+j+1]:
            ax3.plot(x[j:j+2],action_opt[i*48+j:i*48+j+2],color = 'red')
    ...
    #plt.show()
    plt.savefig('D:\\IAQresult\\train_KPL8Z9_strategy2\\day'+str(i+1)+'.svg',dpi = 900)
